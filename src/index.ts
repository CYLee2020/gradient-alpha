 ...
 */
    ...
    radial< T extends HTMLImageElement|ImageData>(img:T, stops: number[] | Array<[number, number | string]> = [0, 1],origin: (number|string)[] = [0.5,0.5], alphas?:number[]):T{
        if (stops.length < 2) {
            return img;
        }       
        let imageData:ImageData,canvas:HTMLCanvasElement|null=null;
        ...
        let originNew=origin.map((item, index: number) => {
        ...
        let xo=width*originNew[0];
        let yo=height*originNew[1];
        let maxLen=Math.max(xo,yo,width-xo,height-yo);
        for (let y0 = 0; y0 < height; y0++) {
